...
class _IconSliderState extends State<IconSlider> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return new _IconSliderRenderObjectWidget(
        ...
        label: widget.label,
        activeColor: widget.activeColor ?? theme.colorScheme.secondary,
...
class _RenderIconSlider extends RenderConstrainedBox {
  ...
  set label(String? newLabel) {
    ...
    if (newLabel != null && newLabel.isNotEmpty) {
      _labelPainter
        ..text = new TextSpan(
          style: _textTheme.bodyLarge?.copyWith(fontSize: 10.0),
          text: newLabel,
        )
        ..layout();
    } else {
  ...
  set activeColor(Color? value) {
    ...
    _activeColor = value ?? Colors.blue;
    markNeedsPaint();
  }
    ...
    if (isInteractive) {
      ...
      _currentDragValue =
          (globalToLocal(details.globalPosition).dx - _kReactionRadius) /
              _trackLength;
      onChanged?.call(_discretizedCurrentDragValue);
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    if (isInteractive) {
      _currentDragValue += (details.primaryDelta ?? 0.0) / _trackLength;
      onChanged?.call(_discretizedCurrentDragValue);
    }
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_reaction.status != AnimationStatus.dismissed) {
      final int? localDivisions = this.divisions;
      if (localDivisions != null) {
        const double tickWidth = 2.0;
        final double dx = (trackLength - tickWidth) / localDivisions;
        ...
        if (dx >= 3 * tickWidth) {
          ...
          for (int i = 0; i <= localDivisions; i += 1) {
      ...
      if (label != null) {
        ...
        if (_labelPainter.text != null) _labelPainter.layout();
  ...
  void _increaseAction() {
    if (isInteractive) {
      onChanged?.call((value + _semanticActionUnit).clamp(0.0, 1.0));
    }
  }
