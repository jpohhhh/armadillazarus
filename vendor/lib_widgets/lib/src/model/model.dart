...
class ModelFinder<T extends Model> {
  ...
  T of(BuildContext context, {bool rebuildOnChange = false}) {
    // Migration note: the old shell used the now-removed
    // inheritFromWidgetOfExactType / ancestorWidgetOfExactType APIs.
    // Keep the original semantics here:
    // - rebuildOnChange=true  => establish a dependency
    // - rebuildOnChange=false => read without dependency
    final inherited = rebuildOnChange
        ? context.dependOnInheritedWidgetOfExactType<_InheritedModel<T>>()
        : context
            .getElementForInheritedWidgetOfExactType<_InheritedModel<T>>()
            ?.widget as _InheritedModel<T>?;

    assert(
      inherited != null,
      'No ScopedModel<$T> found above this BuildContext.',
    );
...
class ScopedModel<T extends Model> extends StatelessWidget {
  ...
  /// Constructor.
  const ScopedModel({Key? key, required this.model, required this.child})
      : super(key: key);
...
class _ModelListener extends StatefulWidget {
  ...
  const _ModelListener({required this.model, required this.builder});
