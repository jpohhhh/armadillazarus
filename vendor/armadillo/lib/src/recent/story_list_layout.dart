...
class StoryListLayout {
  ...
  late double _baseHorizontalGrid;
  late double _baseVerticalGrid;
  late double _horizontalGap;

  /// The factor to scale stories within a row by based on their respective
  /// interaction minutes.
  late double _intraStoryInteractionScaling;

  /// true if we should lay out in multicolumn mode.
  late bool _multiColumn;

  /// [size] is the max size constraints of the list the stories will be laid
  /// out in.
  StoryListLayout({required this.size}) {
  ...
  List<StoryLayout> layout({
    required List<StoryCluster> storyClustersToLayout,
    ...
    if (!_multiColumn) {
      ...
      for (int i = 0; i < stories.length; i++) {
        ...
        story.offset = new Offset(
            (row.isEmpty) ? 0.0 : previousStory!.right + _horizontalGap,
            rowTop);
      ...
      for (List<_StoryMetadata> row in rows) {
        ...
        for (_StoryMetadata story in row) {
          ...
          if (previousRow != null) {
            final currentPreviousRow = previousRow;
            ...
            for (_StoryMetadata intersectingStory in currentPreviousRow.where(
...
class _StoryMetadata extends StoryLayout {
  ...
  _StoryMetadata({
    required this.interactionMinutes,
    required this.jugglingMinutes,
  });
